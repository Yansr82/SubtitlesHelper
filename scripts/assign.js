    ...
    if (savedOptions) {
        ...
        options.forEach((option, index) => {
            if (selectOptions[index]) {
                const selectedIndex = Array.from(selectOptions[index].options).findIndex(opt => opt.text === option);
                if (selectedIndex !== -1) {
...
let selectOptions = document.querySelectorAll('#tools-assign-time-people select');
...
function calculateTimeAndGenerateOutput(hoursValue, minutesValue, secondsValue, nopValue, episode, assignTimeOutput) {
    ...
    if (selectOptions && selectOptions.length > 0) {
        ...
        for (let i = 0; i < nopValue; i++) {
            ...
            if (selectOptions[i % selectOptions.length] && selectOptions[i % selectOptions.length].options) {

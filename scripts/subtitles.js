        ...
        data.forEach((item) => {
          ...
          if (item.number >= 0 && item.number < 300) {
            ...
            if (
              ...
              existingItem.correct !== item.correct ||
              existingItem.annotation !== item.annotation ||
...
function checkTimeCode() {
  ...
  if (lines.length >= 5) {
    const firstLineTimeCodeValid = /^(?:[0-9]{2}:){3}[0-9]{2}$/.test(
      lines[0].substring(0, 11)
    );
    const secondLineTimeCodeValid = /^(?:[0-9]{2}:){3}[0-9]{2}$/.test(
      lines[1].substring(0, 11)
    );
    const thirdLineTimeCodeValid = /^(?:[0-9]{2}:){3}[0-9]{2}$/.test(
      lines[2].substring(0, 11)
    );
    const fourthLineTimeCodeValid = /^(?:[0-9]{2}:){3}[0-9]{2}$/.test(
      lines[3].substring(0, 11)
    );
    const fifthLineTimeCodeValid = /^(?:[0-9]{2}:){3}[0-9]{2}$/.test(
  ...
  for (let index = 0; index < endIndex; index++) {
    ...
    if (prevTimeCode !== null) {
      ...
      if (
        timeCodeToSeconds(currentTimeCode) <= timeCodeToSeconds(prevTimeCode)
      ) {
        ...
        countCharacters(lines[index - 1]) >= 12 &&
        timeCodeToSeconds(currentTimeCode) - timeCodeToSeconds(prevTimeCode) >=
          7
      ) {
  ...
  if (
    lastLine.trim().toLowerCase().endsWith("end") &&
    firstLine.trim() === "01:00:00:00"
  ) {
    const listItem = document.createElement("p");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addWordForm.addEventListener("submit", function (event) {
    ...
    const annotation = document
      .getElementById("floating-annotation")
      .value.trim();
  ...
  function findExistingRow(data) {
    ...
    for (let i = 0; i < tableRows.length; i++) {
      ...
      if (
        td[0].textContent.trim() === data.word &&
        (hasAllClass || rowCategory === dataCategory)
      ) {
        return row;
      }
    }
  ...
  function saveData(newRow) {
    ...
    const existingIndex = rowDataArray.findIndex(
      (item) => item.word === rowData.word && item.category === rowData.category
    );
  ...
  trsWithClass.forEach((tr) => {
    ...
    tr.addEventListener("click", function () {
      const rowNumberClass = Array.from(this.classList).find((cls) =>
        cls.startsWith("row-")
      );
      ...
      const indexToRemove = rowDataArray.findIndex(
        (item) => item.number === rowNumber
      );
      if (indexToRemove !== -1) {
